document.addEventListener("DOMContentLoaded", function () {
    ...
    if (getCartItem.length > 0) {
        ...
        for (let i = 0; i < getCartItem.length; i++) {
            ...
            cartProd += `<div class="cart-qty" >`
            cartProd += `<div class="qty-min"><button onclick="handleRemovevProd(${i})"><i class="fa-solid fa-minus"></i></button></div>`
            cartProd += `<div class="qty-count"><span class="item-count"> ${getCartItem[i].qty} </span></div>`
            cartProd += `<div class="qty-plus"><button class="btn" onclick="handleAddProd(${getCartItem[i].id})"><i class="fa-solid fa-plus"></i></button></div>`
            cartProd += `</div >`
            cartProd += `<div  class="itm-price"><h3 class="cart-product-price">${getCartItem[i].qty} <i class="fa-solid fa-xmark icon-mul"></i> <i class="fa-solid fa-dollar-sign"></i>${getCartItem[i].price}</h3></div>`
            cartProd += `</div>`
...
function handleAddProd(id) {
    ...
    document.getElementsByClassName("cart-product-price")[productIndex].innerHTML = `${product[productIndex].qty} <i class="fa-solid fa-xmark icon-mul"></i> ${product[productIndex].price}`
...
function handleRemovevProd(index) {
    ...
    if (getProduct.length && getProduct.length > 0) {
        ...
        for (let i = 0; i < getProduct.length; i++) {
            ...
            cartProd += `<div class="cart-qty" >`
            cartProd += `<div class="qty-min"><button onclick="handleRemovevProd(${i})"><i class="fa-solid fa-minus"></i></button></div>`
            cartProd += `<div class="qty-count"><span class="item-count"> ${getProduct[i].qty} </span></div>`
            cartProd += `<div class="qty-plus"><button onclick="handleAddProd(${getProduct[i].id})"><i class="fa-solid fa-plus"></i></button></div>`
            cartProd += `</div>`
            cartProd += `<div  class="itm-price"><h3 class="cart-product-price">${getProduct[i].qty} <i class="fa-solid fa-xmark icon-mul"></i> <i class="fa-solid fa-dollar-sign"></i>${getProduct[i].price}</h3></div>`
            cartProd += `</div>`
